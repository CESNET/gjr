    ...
    def handle(self, *args, **options):
        ...
        with open(pulsars_path, mode='r', newline='\n', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                add_galaxy_server(name, latitude, longitude)
        ...
        with open(pulsars_path, mode='r', newline='\n', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                add_pulsar(galaxy_name, pulsar_id, latitude, longitude, description)

            self.stdout.write(self.style.SUCCESS(f"Successfully added pulsars from {pulsars_path}"))

def add_galaxy_server(galaxy_name, latitude, longitude):
    ...
    if created:
        self.stdout.write(self.style.SUCCESS(f"Created galaxy: {name}"))
    else:
        self.stdout.write(self.style.WARNING(f"Galaxy already exists: {name}"))

def add_pulsar(galaxy_name, pulsar_id, latitude, longitude, description):
    ...
    if created:
        self.stdout.write(self.style.SUCCESS(f"Created pulsar: {name}"))
    else:
        self.stdout.write(self.style.WARNING(f"Pulsar already exists: {name}"))
