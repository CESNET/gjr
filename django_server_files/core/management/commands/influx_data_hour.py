...
class Command(BaseCommand):
    ...
    def most_used_tools_influxdb_response_to_dict(self, destination_dict):
        ...
        # Temporary dictionary to hold all tool data before sorting
        temp_destination_data = {}

        # Process each record in the values
        for value in values:
            # Use indices for the expected columns
            _, destination_id, _, job_count, tool_id = value

            destination_id = destination_id if "pulsar" in destination_id else "eu_pbs"

            if destination_id not in temp_destination_data:
                temp_destination_data[destination_id] = []

            # Append each tool's data to temporary list
        ...
        for destination_id, tools in temp_destination_data.items():
            # Sort tools by job count in descending order
    ...
    def num_user_running_jobs_influxdb_response_to_dict(self, destination_dict):
        ...
        # Temporary storage for job counts by user
        temp_user_data = {}
        ...
        for value in values:
            ...
            # Increment the job count for each user
            temp_user_data[destination_id][user_id] += job_count

        # Process and store the top 5 users by job count for each destination
        for destination_id, users in temp_user_data.items():
            # Convert the user data to a list of dicts and sort by job_num
    ...
    def unique_users_influxdb_response_to_dict(self, destination_dict):
        ...
        # Iterate over each record in the 'values' array
        for value in values:
            ...
            # Store the unique user count for the destination
